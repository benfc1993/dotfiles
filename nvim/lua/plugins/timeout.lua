...
local WriteToFile = function()
	...
	local file = io.open(os.getenv("HOME") .. "/.config/timeout.txt", "w")
	if not file then
		return
	end
	file:write(time)
...
local endBreak = function(float)
	...
	if shouldRun then
		WriteToFile()
		vim.defer_fn(M.StartTimeOut, 1000)
	end
end
	...
	countdown:start(
		...
		vim.schedule_wrap(function()
			...
			vim.api.nvim_buf_set_lines(
				...
				5,
				false,
				{ "Time to take a break", "This is not a drill!", "Take a break for at least", m .. ":" .. secondsText }
			)
		end)
	)
	local timer = vim.loop.new_timer()
	timer:start(
		math.floor(vim.g.timeoutMinBreakMins * 60 * 1000),
		0,
		vim.schedule_wrap(function()
			timer:close()
			countdown:stop()
